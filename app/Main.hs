...
handleOptionPricing optionType pricingFunction = do
  liftIO $ putStrLn "Received request for option pricing"
  req <- jsonData :: ActionM OptionRequest
  let OptionRequest numSims s k r v t = req
  price <- liftIO $ pricingFunction numSims s k r v t
...
main :: IO ()
main = scotty 3000 $ do
    post "/price/digital-call" $ handleOptionPricing "digital call" monteCarloDigitalCallPrice
    post "/price/digital-put" $ handleOptionPricing "digital put" monteCarloDigitalPutPrice
